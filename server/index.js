...
io.on('connection', (socket) => {
  ...
  const joinRoom = (room, name, avatar) => {
  ...
  socket.on('create-room', ({ name, avatar, settings }) => {
  ...
  socket.on('quick-play', ({ name, avatar }) => {
  ...
  socket.on('chat-message', ({ text }) => {
    ...
    const payload = { playerId: socket.id, playerName: player.name, avatar: player.avatar, text };
